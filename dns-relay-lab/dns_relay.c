...
static void initialize() {
  ...
  if (ret < 0)
    handle_fatal("bind() error");

  epoll_fd = epoll_create1(0);
  if (epoll_fd < 0)
    handle_fatal("epoll_create1() error");

  ret = epoll_ctl(epoll_fd, EPOLL_CTL_ADD, listen_fd, listen_event(listen_fd));
  if (ret < 0)
    handle_fatal("epoll_ctl() error");

  upstream_fd = socket(PF_INET, SOCK_DGRAM, 0);
  if (upstream_fd < 0)
    handle_fatal("socket() error");

  ret = epoll_ctl(epoll_fd, EPOLL_CTL_ADD, upstream_fd,
                  listen_event(upstream_fd));
  if (ret < 0)
    handle_fatal("epoll_ctl() error");
}
...
int main() {
  ...
  assert(DNS_QUESTION_SIZE ==
         4); // dns question except domain name must be 4 bytes
  ...
  while (1) {
    ...
    for (int i = 0; i < event_count; i++) {
      ...
      int len =
          recvfrom(event_fd, buf, MAX_DATAGRAM_BUFFER_SIZE, 0,
                   (struct sockaddr *)&client_info.addr, &client_info.len);
      if (len < 0)
        handle_fatal("recvfrom() error");
      ...
      /*
          TODO: implement the logic about how to handle datagrams from listen
         socket Note:
              1. before finishing this part, make sure that you have finished
         other TODO parts
              2. functions in dns_dgram_utils.c and dns_relay_utils.h may help a
         lot
              3. remember to call log_result() after sending a datagram
          Important:
              1. in this lab, consider that the dns request datagram contains
         ONLY one entry in question section for convenience.
      */
      parse_question_section(name, &question, buf);
      int ip_count = try_answer_local(
          ip, name,
          "/home/kyomoto/repo/cpp/cn-2025-labs/dns-relay-lab/mappings");
      if (ip_count > 0) {
        int response_len =
            transform_to_response(buf, len, ip, ip_count, &question);
        send_datagram(listen_fd, buf, response_len, client_info);
